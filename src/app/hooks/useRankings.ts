...
import { Ranking, Media } from '@/lib/definitions/index'
...
export const useRankings = (): UseRankingsReturn => {
  ...
  const addRanking = useCallback(
    async (rank: number, notes?: string, media?: Media) => {
      if (!user) throw new Error('User not authenticated')

      try {
        // Check if already ranked
        const { exists, ranking: existingRanking } =
          await RankingService.isMediaRanked(user.uid, media?.id || '')
  ...
  const checkIfRanked = useCallback(
