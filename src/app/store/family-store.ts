...
export const useFamilyStore = create<FamilyStore>((set, get) => ({
  ...
  fetchUserFamilies: async (userId: string) => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Failed to fetch families'
      set({ error: errorMessage, loading: false })
    }
  },
  ...
  addFamilyMember: async (
    ...
    try {
      ...
      set((state) => ({
        ...
        currentFamily:
          state.currentFamily?.id === familyId
            ? {
                ...state.currentFamily,
                memberIds: [...state.currentFamily.memberIds, userId],
              }
            : state.currentFamily,
        loading: false,
      }))
      ...
      set((state) => ({
        ...
        currentFamily:
          state.currentFamily?.id === familyId
            ? {
                ...state.currentFamily,
                memberIds: state.currentFamily.memberIds.filter(
                  (id) => id !== userId
                ),
              }
            : state.currentFamily,
        familyMemberRoles: state.familyMemberRoles.filter(
          (role) => !(role.familyId === familyId && role.userId === userId)
        ),
        loading: false,
      }))
    } catch (error) {
  ...
  deleteFamily: async (familyId: string, userId: string) => {
    ...
    try {
      ...
      set((state) => ({
        ...
        currentFamily:
          state.currentFamily?.id === familyId ? null : state.currentFamily,
        familyMemberRoles: state.familyMemberRoles.filter(
          (role) => role.familyId !== familyId
        ),
        loading: false,
      }))
    } catch (error) {
