...
export const MediaCard = ({ media, onAddToRankings, isRanked }: MediaCardProps): React.ReactNode => {
  const Icon = getMediaIcon(media.type);

  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
      <div className="aspect-[2/3] bg-gray-200 dark:bg-gray-700 relative">
        <Image src={media.poster || ''} alt={media.title} className="w-full h-full object-cover" width={80} height={120} />
        {media.rating && (
      ...
      <div className="p-4">
        ...
        >
          {!isRanked ? <Plus className="w-4 h-4" /> : <Check className="w-4 h-4" />}
